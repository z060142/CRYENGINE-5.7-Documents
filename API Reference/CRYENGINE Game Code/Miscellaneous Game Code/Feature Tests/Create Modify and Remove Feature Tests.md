...
TrySpawnPlayer:
**
 Tell the player to spawn in (if not spawned in already):
...
SetItem:
**
 Make the player select a particular item from his inventory:
...
SetAmmo:
**
 Changes the ammo count of the current selected item. "ammo" is the ammo count on the inventory and "clip" is the number of rounds on the clip:
...
SetSuitMode:
**
 Put the player into a particular suit mode:
...
**
 Move a player next to another entity in the level:
...
**
 Send local player's input handler a button press message:
...
**
 Send local player's input handler a button release message:
...
OverrideAnalogInput:
**
 Send local player's input handler a change to an analogue input value:
...
DoMenuCommand:
**
 Execute a menu command (actions normally activated by clicking buttons etc. in the front-end or in-game menu):
...
WatchCCCPoint:
**
 Start counting the number of times a code coverage checkpoint is hit:
...
**
 Reset how many times each code coverage checkpoint is hit to 0:
...
**
 Verify the number of times a code coverage checkpoint is hit is a specific number (fail feature test if not):
...
**
 Mark hitting a specific code coverage checkpoint as having a particular response (restartTest / restartSubroutine / failTest / passTest / expectedNext):
...
WaitSingleFrame:
**
 Wait until a frame has gone by:
...
**
 Wait until a player (or an entire set of players) is alive:
...
**
 Wait until all code coverage checkpoints marked as "expected" have been hit:
...
**
 Wait until a player (or an entire set of players) has a particular target type:
...
RunFeatureTest:
**
 Pause execution or this feature test, run another, then continue with this one if that one succeeds:
...
**
 Run all flowgraph feature tests which exist in the loaded level:
...
The top level XML tag. Can include a <
**
Settings
**
...
Defines a set of feature tests with the specified set name. Should include zero or more <
**
FeatureTest
**
> sections.
