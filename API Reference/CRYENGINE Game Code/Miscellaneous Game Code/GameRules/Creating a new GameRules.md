...
CryENGINE already provides several basic GameRules implementations, including *Singleplayer* and * DeathMatch*. It's possible to derive from these GameRules to reuse their core functionality.
...
The GameRules can now simply be started by typing **sv_gamerules CaptureTheFlags** in the console of the server or in the launcher game.

### GameRulesModulesManager

CryENGINE uses a 'GameRulesModulesManager' to parse an XML file containing definitions of each of the gamerules at runtime, the XML file is located at `Game/Scripts/GameRules/GameModes.xml` and contains the name of each gamemode, its aliases, level location directories, a path to a specific XML file that determines the behaviour of the gamemode and finally, a specific HUD, if required.
