...
`
Localization\<Language>\text_ui*.xml
`
for it to be processed to the default locations used by the engine.

For UI translation the tables must have a column
**
"KEY"
**
,
...
A label is also translated if it is passed as string to a dynamic textfield via:
`
Code\FlowGraph\LUA
`
...
one
**
 font library for all flash assets. One reason is to save memory since you don't want to embed every font in every flash file. This technique also allows embedding different glyph sets for different languages.
...
In the GameSDK sample project, we use the library
`
...
For each distinct font you wish to use, create new font symbols and set the font-style in the library (
**
Right-Click -> New font
**
).
...
`
 file for that specific language.

Now, use
`
gfxexport
`
 tool to convert the font library to Scaleform GFX format.
...
Make sure to copy the resulting GFX into your per-language localized asset file (so it eventually ends up in
`
Localization/<language>_xml.pak
`
 for that language, for the default asset build script you would store it as
`
Localization/<language/HUD_Font_LocFont.gfx)
`
