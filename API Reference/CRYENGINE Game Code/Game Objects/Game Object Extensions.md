...
A third CGameObjectExtensionHelper parameter is available, and denotes the maximum number of**[RMIs](../../CRYENGINE%20Engine%20Code/Engine%20Modules/CryNetwork/RMI%20Functions.md)** supported by this game object.

The important functions to implement the most basic Game Objectextension are the following:

**Function** | **Description**
--- | ---
IGameObjectExtension::Init() | The Init function should set the member values and validate with the return value if the initialization is successful. If needed, resources should be allocated during the execution of the Init.
IGameObjectExtension::PostInit() | The ideal location to set the initial update slot state (more information can be found in IGameObject::EnableUpdateSlot()).
IGameObjectExtension::Release() | This function should release any resource previously allocated by the extension. Additionally, the Release function need to free the object instance.
IGameObjectExtension::Serialize() | Any critical state variables needs to be serialized, both for disc serialization (i.e.: save games) or network serialization.
IGameObjectExtension::Update() | Any frame rate dependent updates should be implemented as part of this function.
...
To see how this system is working, you can always use g_showUpdateState 2. It's showing all of the details about why a particular entity has been enabled. In order to do the visibility and range checks to evaluate the update rules, the following is done:
