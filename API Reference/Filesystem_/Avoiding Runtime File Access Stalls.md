...
It is possible to track invalid file reads (occurring in the game) during run-time. The error message
*
'Invalid File Access'
*
 means reading or trying to open files from a thread which is not the streaming thread. These file access operations can cause severe stalls.
...
FileAccessLogs
*
 dir (XenonStackParse uses the working directory to search for the .pdb files). This will create a directory called processed, which will have a file containing the resolved callstack for each of the logged files.
