...
[Accessing an Entity from a Level](#accessing-an-entity-from-a-level)[Spawning an Entity](#spawning-an-entity)[Positioning and Moving an Entity](#positioning-and-moving-an-entity)[Rotation](#rotation)
Related Content:
...
There are two common ways to move an Entity. Either by manually moving an object by updating the **`Position`** property, or moving the object by setting the **` Physics.Velocity`** property. For more information on how to use the velocity approach please refer to [Physics Interaction](06.%20Physics%20Interaction.md).
...
**`Speed`** is defined in meters per second (m/s) and is represented as a three dimensional vector. However, to ensure a constant move rate across the screen (because all PC’s have different frame rate performances) [FrameTime](01.%20Architectural%20Overview.md#01.ArchitecturalOverview-FrameTime) normalizes the speed using the time that it took to render the previous frame.
...
Again, we are normalizing the rotation speed with [FrameTime](01.%20Architectural%20Overview.md#01.ArchitecturalOverview-FrameTime) to ensure smooth movement.
