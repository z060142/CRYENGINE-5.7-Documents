...
CCrySignal
**
 wherever you need to observe something. This is our own implementation of signals (see Qt signals, or boost::signal) which is much more flexible than writing your own observer system. Do not use a virtual interface for observers, it’s 2016, use a functional approach, hence CCrySignal.
...
`
//Including UI headers where they shouldn't be is a bad sign
`
`
#include “SomeUIClass.h”
`
`
...
`
`
//This should not have a direct reference to the UI, but should simply call a signal to notify potential observers
`
`
...
`
`
//this will trigger the signal value changed which will call OnValueChanged()
`
`

`
`
m_system->SetValue(val);
`
`
...
`
`
//Callback registered to signal on m_system
`
`
void
`
...
`
`
//avoid updating value again here since we already did when the action changed
`
`
