...
*Before 3.6.2:* To compile the code provided, you need to have Microsoft Visual C++ 2010 installed. * Starting with 3.6.2:* To compile the code provided, you need to have Microsoft Visual C++ 2012 installed. * Starting with 3.8.4*: Refer to **[Visual Studio Supported Versions_dup](/docs/static/engines/cryengine-5/categories/23756813/pages/23306511)**.
...
[3rd party SDK dependencies](#3rd-party-sdk-dependencies)[Solution Files](#solution-files)[Compilation Configurations](#compilation-configurations)
...
**DirectX SDK** | *Before 3.7.0*: June 2010 ****Starting with*** 3.7.0:* No longer used | `Code\SDKs\DXSDK` | *Before 3.7.0*: Required. ****Starting with***3.7.0*: No longer used **[Download Link](http://www.microsoft.com/en-us/download/details.aspx?id=6812)**
**Intel GPA** | *Before 3.6.1*: 2012 R15 ****Starting with***3.6.1*: 2014 R2 | `Code\SDKs\GPA` | Optional, controlled by **CRY_PROFILE_MARKERS_USE_GPA** By default, this SDK is used. **[Download Link](http://registrationcenter-download.intel.com/akdlm/irc_nas/4211/gpa_14.2_release_225646_windows.exe)**
...
**OSVR** | ****Starting with***5.0.0*: 0.6 ****Starting with*** 5.1.0:* Included by default | `Code\SDKs\OSVR` | Optional, presence automatically detected by WAF.
...
**AMD GPU Services** | **Starting with* 3.8.1*: 2.1 | `Code\SDKs\AMD\AGS Lib` Extra, starting with 5.0.0:` Code\SDKs\AMD\AMD_Extensions` | **[Download Link](http://developer.amd.com/tools-and-sdks/graphics-development/amd-gpu-services-ags-library/)** * Starting with 5.0.0:* The AMD_Extensions folder can be obtained within an AMD Code sample: **[Download Link](http://developer.amd.com/wordpress/media/2013/05/DepthBoundsTest11_v1.0.zip)**
**NVAPI** | **Starting with* 3.8.1*: r343 | `Code\SDKs\NVAPI` | **[Download Link](https://developer.nvidia.com/nvapi)**
...
**XOne XDK** | *Please check **[Xbox One Supported Versions](/docs/static/engines/cryengine-5/categories/23756813/pages/23306689)*** | `Code\SDKs\DurangoSDK` | Only required when targeting XBox One.
**PS4 SDK** | *Please check **[PS4 SDK versions](/docs/static/engines/cryengine-5/categories/23756813/pages/23306679)*** | `Code\SDKs\Orbis` | Only required when targeting PS4.
**PS4 Pad Support** | *Please check **[PS4 SDK versions](/docs/static/engines/cryengine-5/categories/23756813/pages/23306679)*** | `Code\SDKs\OrbisPad` | *Starting with 3.6.4:* Only required when targeting Win64 while having your SDK configured with PS4 support
...
Rebuilding of the Sandbox is not required as long as the interfaces declared in CryCommon are not changed. In that case, you can use the pre-built binaries from the CRYENGINE SDK package. If this doesn't happen during development, you wont need to rebuild the Sandbox, and the SDKs listed here needn't be obtained.

Dependency | Version | Path | Notes
--- | --- | --- | ---
**FbxSdk** | *Before 3.6.2*: 2013.3 * Starting with 3.6.2*: 2014.2 * Starting with 3.8.4*: 2016.1 | *Before 3.6.2`:`*` Code\SDKs\FbxSdk\2013.3`* Starting with 3.6.2:*` Code\SDKs\FbxSdk` | Required. Also used by Resource Compiler. **[Download Link](http://usa.autodesk.com/adsk/servlet/pc/item?siteID=123112&id=10775892)**
**Xtreme ToolkitPro** | 13.4.0 | `Code\SDKs\XT_13_4` | Required. **[Note on building XT Toolkit](/docs/static/engines/cryengine-5/categories/23756813)**. Can be purchased from [** CodeJock**.](http://www.codejock.com/)

#### Sandbox Plugins

The Sandbox plugins consume SDKs from the `Code\SDKs` folder. Before 3.8.0, the ` Code\Sandbox\SDKs` folder is also used.

Rebuilding of the Sandbox plugins is not required unless you wish to customize them. Instead, you can use the pre-built binaries from the CRYENGINE SDK package. If there is no need for customization, you wont need to rebuild the Sandbox plugins, and the SDKs listed here needn't be obtained.

Dependency | Version | Path | Notes
--- | --- | --- | ---
**Perforce API** | *Starting with 3.6.2*: 2014.1 * Starting with 3.8.5*: 2015.1 | `Code\SDKs\p4api` | Required only for PerforcePlugin.
**FbxSdk** | *Before 3.6.2*: 2012.1 * Starting with 3.6.2*: 2014.2 * Starting with 3.8.4*: 2016.1 | *Before 3.6.2:*`Code\Sandbox\SDKs\FbxSdk`* Starting with 3.6.2:*` Code\SDKs\FbxSdk` | Required only for FBXPlugin. Also used by RC. **[Download Link](http://usa.autodesk.com/adsk/servlet/pc/item?siteID=123112&id=10775892)**
...
Rebuilding of the Resource Compiler is not required unless you wish to customize it. Instead, you can use the pre-built binaries from the CRYENGINE SDK package. If there is no need for customization, you wont need to rebuild the Resource Compiler, and the SDKs listed here needn't be obtained.

Dependency | Version | Path | Notes
--- | --- | --- | ---
**FbxSdk** | *Before 3.6.2:* 2013.3 * Starting with 3.6.2:* 2014.2 * Starting with 3.8.4*: 2016.1 | *Before 3.6.2:* `Code\SDKs\FbxSdk\2013.3`* Starting with 3.6.2:* ` Code\SDKs\FbxSdk` | Required. Also used by Sandbox. **[Download Link](http://usa.autodesk.com/adsk/servlet/pc/item?siteID=123112&id=10775892)**
...
**Alembic** | *Starting with 3.8.4*: 1.5.8 | *Starting with 3.8.4:*`Code\SDKs\Alembic` | Required. **[Download Link](https://github.com/alembic/alembic/releases)**
**hdf5** | *Starting with 3.8.4*: 1.8.15-patch1 | *Starting with 3.8.4:*`Code\SDKs\hdf5` | Required for Alembic. **[Download Link](https://www.hdfgroup.org/ftp/HDF5/releases/hdf5-1.8.15-patch1/src/)**
**ilmbase** | *Starting with 3.8.4*: 2.2.0 | *Starting with 3.8.4:*`Code\SDKs\ilmbase` | Required for Alembic. **[Download Link](http://www.openexr.com/downloads.html)**
**szip** | *Starting with 3.8.4*: 2.1 | *Starting with 3.8.4:*`Code\SDKs\szip` | Required for hdf5. **[Download Link](https://www.hdfgroup.org/HDF5/release/obtain5.html#extlibs)**
...
Rebuilding of the 3rd party plugins is not required unless you wish to customize them. Instead, you can use the pre-built binaries from the CRYENGINE SDK package. If there is no need for customization, you wont need to rebuild the 3rd party plugins, and the SDKs listed here needn't be obtained.
...
Starting with version 3.7.0, the WAF build system is used to generate solutions. Projects and solutions are no longer shipped, you can now regenerate appropriate project files using WAF. Some stand-alone tools still use solution and project files. In this case the appropriate project and/or solution files will be in the Tool's code folder.
...
*Versions 3.7.0 to 3.8.1*: WAF does not target dedicated folders, and this section does not apply to those builds. For these versions, this section only applies to MSBuild-based builds.
