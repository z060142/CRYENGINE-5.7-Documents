...
Mentioned functions Save/Load functions are wrappers around IArchiveHost interface, instance of which is located in gEnv->pSystem->GetArchiveHost(). Alternatively, if you have direct access to archives code (if they are located in the same modules, e.g. in CrySystem or EditorCommon) you could use archive classes directly:
...
^ | Inline | Inline property on the same line as name of the structure root. Can be used to put fields in one line in horizontal layout, rather than default vertical list.
...
**Callback** | Provides per-property callback function. See [Adding Callbacks to PropertyTree](Serialization%20Library.md#SerializationLibrary-AddingCallbackstoPropertyTree) | All types apart from compound ones (structs and containers) |
...
```
- Use Serialization::Callback. This decorator provides gives you opportunity to add callback function for each property. It works only with PropertyTree, and should be used only in Editor code:
```
...
If you want to specify enumeration value I suggest you to use enum registration macro from [Defining Data](Serialization%20Library.md#SerializationLibrary-DefiningData) section.
...
[Features](#features)[Tutorial](#tutorial)[Use cases](#use-cases)
