...
CRYENGINE has a custom reference-counted string class
**
CryString
**
 (declared in
...
CryString.h
*
) which is a replacement for the STL std::string. It should always be preferred over std::string. For convenience,
**
string
**
 is used as a typedef for
`
CryString
`
.
...
By using the suggested method, you avoid allocation/deallocation and copying for a temporary string object. This is a common problem for most string classes. By simply using the macro
`
CONST_TEMP_STRING
`
...
wstring
**
 and never include the standard string header <string>.
...
c_str()
`
 method to access the contents of the string.
...
c_str()
`
 method since strings are reference-counted and a wrong string instance could be affected.
...
-
For fixed size strings (e.g. 256 chars) use
`
CryFixedStringT
`
