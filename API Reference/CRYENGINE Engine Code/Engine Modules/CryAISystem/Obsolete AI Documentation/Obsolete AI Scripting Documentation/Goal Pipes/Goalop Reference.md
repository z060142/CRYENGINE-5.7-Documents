...
goalpipe
*
 is a sequence of tasks/commands (
*
goalops
*
) to be performed by an AI. In effect, it is a simple virtual machine which allows you to put together building blocks of actions and execute them over a number of frames.
...
Goalpipes can be created dynamically, but are usually created once while initializing. They can be executed once, or allowed to loop. During goalpipe creation, when goals are added to a pipe using AI.PushGoal, they can be made
*
blocking
*
 or
...
Default values for parameters are typed in
**
bold
**
.
...
Goalop parameter "blocking" should either be 0 or 1 in Lua, or
*
false
*
 or
...
Lua:
**
 AI.PushGoal("acqtarget", blocking,
*
AI object's name or a
*
...
Lua:
**
 AI.PushGoal("adjustaim", blocking,
*
aim / hide, use LastOp as backup, allow prone, timeout
*
)
...
*
timeoutRandomness="timeout randomness"
*
/>
...
0
**
 to aim (default), 1 to hide

*
use LastOp as backup
*
:
**
0
...
XML:
**
 <Animation mode="Action/Signal" name="name"/>

**
Parameters:
**

mode: AIANIM_SIGNAL (to play one-shot animations) or AIANIM_ACTION (to play looping animations)

name: name of animation action or signal to set
...
startArcAngle="number"

*
approachRadius="number (default is
*
...
startWidth, directionTolerance, startArcAngle, approachRadius (
**
0
**
 by default): please see documentation on navigational smart objects for details.
...
Lua:
**
 AI.PushGoal("approach", blocking, approach distance or time,
*
flags, end accuracy
*
)
...
distance="number" or time="number"
...
approach distance / time: use flag AI_USE_TIME to specify time, rather than distance
...
0
**
 by default) allowable movement end accuracy
...
Make a small evasive movement – back, left, right, etc.
Make sure this one is reset, don't allow it to loop.

**
Lua:
**
 AI.PushGoal("backoff", blocking, distance,
*
...
AI_BACKOFF_FROM_TARGET,

AI_MOVE_TOWARDS_GROUP,

AI_CHECK_SLOPE_DISTANCE,
...
Lua:
**
 AI.PushGoal("bodypos", blocking, body state,
*
delayed
*
)
...
XML:
**
 <BodyPos id="see the table below"
*
delayed="
*
**
...
Lua:
**
 AI.PushGoal("communicate", blocking, communication name, channel name,
*
expirity, minSilence, timeout, ignoreSound, ignoreAnim
*
)
...
XML:
**
 <Communicate name="communication name" channel="channel name"
*
timeout="number (
*
**
...
communication name: the name of the actual communication. Possible communications are defined by the XML file that the property
*
CommConfig
*
 of this AI's Lua script refers to.

channel name: the name of the communication channel this AI is using. The channel should be defined in any XML file which resides in
*
...
0
**
 by default) maximum allowable delay in triggering this communication (when the communication channel is temporarily occupied). If the communication couldn't be triggered within this time period, it is discarded.
...
-1
**
 by default, which means "use the default value set for the communication channel used") minimum time (in seconds) for which the channel remains occupied after the communication has completed
...
8
**
 by default) maximum time this goalop is allowed to execute
...
false
**
 by default) disable playing animation
...
false
**
 by default) ordered requests get serviced before unordered ones
...
Lua:
**
 AI.PushGoal("devalue", blocking,
*
also devalue puppets, clear [the list of] "devalued"
*
)
...
Lua:
**
 AI.PushGoal("firecmd", blocking, fire mode,
*
use LastOp, min timeout, max timeout
*
)
...
XML:
**
 <FireCmd mode="see the table below"
*
useLastOp="
*
**
...
**
*
/true" timeout="number" or timeoutMin="number" timeoutMax="number"
*
/>
...
Lua:
**
 AI.PushGoal("followpath", blocking, pathfind to start, reverse path, start nearest, number of loops,
*
end accuracy, use point list, avoid dynamic obstacles
*
)
...
XML:
**
 <FollowPath loops="number"
*
pathFindToStart="false/
*
**
*
true
...
pathfind to start: false (go to the start of the path in a straight line) or true

reverse path: false or true (navigate the path from the last point to the first)

start nearest: false (start from the
*
first
*
 point of the path) or true (start from the
*
nearest
*
 point of the path)
...
Lua:
**
 AI.PushGoal("hide", blocking, search distance, hide method,
*
exactly, min distance, look at LastOp
*
)
...
*
evaluationMethod="number (see Lua section)"
*
...
search distance: distance within which to search for a hide (

hide method: one of the following:
...
Lua:
**
 AI.PushGoal("ignoreall", blocking, false/
**
true
**
)
...
Find a special object – e.g. probtarget, atttarget, refpoint – or a named AI object, and set as LastOp, within a given (if given)
*
range
*
. Commonly followed by AcqTarget to gain as attention target.
...
Lua:
**
 AI.PushGoal("lookaround", blocking, look around range,
*
scan range, interval
*

*
min, interval max, flags
*
)
...
*
interval="number" or intervalMin="number"
*
...
0
**
 (default), either of the following can be set: AI_BREAK_ON_LIVE_TARGET, AILASTOPRES_USE
...
Lua:
**
 AI.PushGoal("lookat", blocking, start angle, end angle,
*
use LastOp, flags
*
)
...
*
angleThreshold="number (default is
*
...
0
**
 (default), either of the following can be set: AI_LOOKAT_CONTINUOUS, AI_LOOKAT_USE_BODYDIR
...
*
by default), direction only distance (
*
**
*
0
...
*
endTolerance="number (default is
*
...
*
directionOnlyDistance="number (default is
*
...
false
**
 by default) place the pathfinding request at the beginning of the request queue - can be useful e.g. for running away from grenades.
...
Lua:
**
 AI.PushGoal("proximity", blocking, radius, signal name,
*
flags
*
)
...
XML:
**
 <Proximity radius="number" signal="signal name"
*
signalWhenDisabled="
*
**
...
0
**
 (default), AIPROX_VISIBLE_TARGET_ONLY, AIPROX_SIGNAL_ON_OBJ_DISABLE
...
(Alias: speed) Set movement speed – walk, run, sprint etc. If
*
"max length"
*
 is given, "urgency" becomes scaled based on "length" and
*
"max length"
*
.
...
Lua:
**
 AI.PushGoal("run", blocking, max urgency,
*
min urgency, max length
*
)
...
Lua:
**
 AI.PushGoal("seekcover", blocking, hide / unhide, radius,
*
iterations, LastOp options
*
)
...
hide / unhide: COVER_HIDE / COVER_UNHIDE

radius: seek the locations within this distance
...
3
**
 by default) the number of iterations
...
1 (set bit '0') to use LastOp if the attention target does not exists;

add 2 (set bit '1') to prefer areas towards LastOp
...
Lua:
**
 AI.PushGoal("signal", blocking, signal ID,
*
signal text, destination filter,
*
...
*
name="signal name (empty string by default)"
*
...
Lua:
**
 AI.PushGoal("stick", blocking, stick distance or duration (controlled by
*
flags
*
),
*
flags, stick flags, end accuracy, end distance variation
*
)
...
duration="number" or distance="number"
...
*
distanceVariation="number (default is
*
...
*
locateName="AI object's name or a
*
...
*
regenDistThreshold="number (default is
*
...
stick distance or time: use flag AI_USE_TIME to specify time, rather than distance
...
0
**
 by default) use STICK_BREAK for "non-continuous", rather than "continuous" stick;

use STICK_SHORTCUTNAV to try shortcut navigation (i.e. direct connection)
...
distance
**
) allowable movement end accuracy

*
end distance variation
*
:
**
0
...
Lua:
**
 AI.PushGoal("strafe", blocking, distance start,
*
distance end, strafe while moving
*
)
...
**
*
/true" distance= or distanceStart="number (default is
*
...
distance start: strafe from the beginning up to this length of the path
...
0
**
 by default) strafe within this distance from the end of the path
...
Lua:
**
 AI.PushGoal("timeout", blocking, min time,
*
max time
*
)
...
XML:
**
 <Timeout interval= or intervalMin="number"
*
intervalMax="number (default is
*
...
follow exactly
*
: 0 (default) / 1 (follow the path exactly, possibly looking at the target all the time)

end accuracy: (default is
**
0
**
) allowable movement end accuracy
...
Lua:
**
 AI.PushGoal("usecover", blocking, hide / unhide, min time,
*
max time,
*
...
Lua:
**
 AI.PushGoal("wait", blocking, whom)
