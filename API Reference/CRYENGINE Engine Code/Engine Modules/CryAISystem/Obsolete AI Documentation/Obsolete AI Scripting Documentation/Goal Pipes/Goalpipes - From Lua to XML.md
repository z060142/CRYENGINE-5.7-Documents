...
AI.BeginGoalPipe and AI.EndGoalPipe are naturally replaced by tag <GoalPipe>. Keep in mind that XML goalops are **blocking** by default, so there is no need to add attribute ** blocking="true"** to each blocking goalop.
