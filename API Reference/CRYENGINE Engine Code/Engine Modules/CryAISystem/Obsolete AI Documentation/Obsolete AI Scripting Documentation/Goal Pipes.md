...
Goals can be atomic and derived. An **atomic** goal is a simple goal that cannot be further split into sub-goals, and is one logical unit. A ** derived** goal is a goal that is a combination of a few atomic or derived goals that can execute in parallel or in a sequence. While atomic goals are defined by the system and introduction of new atomic goals would require working on the low-level AI system, the users can define derived goals themselves. The definition of new derived goals can be made on the C++ side or in Lua (both contain a set of functions that work with goals). Usually, goals are exclusively defined in Lua script.

Since derived goals are created by combining atomic and other derived goals, there must be a way to organize a few goals into a logical sequence – a program – that has a start point, a body to execute and conditions to end. This logical union exists in the CryAISystem and is called a **Goal Pipe**.
...
The parameter 6 of the function **SelectPipe** is the one relative to the dynamic parameters. The engine will create an internal structure to store a list of **[parameter_name, parameter_value]** that a specific Goalpipe can interpret later if it's needed.
