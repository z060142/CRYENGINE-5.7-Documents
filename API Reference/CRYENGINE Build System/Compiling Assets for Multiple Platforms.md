...
**
 ...
 |
The folder where the Tools/rc/rc.exe you need to use is located.

Usually it is the root folder of the build you want to compile the assets for.
...
**
 ...
 |
The older where the assets used inside the game are stored.
...
**
 ...
 |
The folder name for the game directory (usually "Game").
...
**
 ...
 |
The root where the compilation process will be executed.
...
RC Job
**
" is a set of operations the RC needs to execute to perform some tasks (in our specific case it describe the asset compilation process).

Every "
**
RC Job
**
" is defined as an xml file.
...
**
 ...
 |
This is the name of the game folder where the rc.exe can find your game assets.
...
**
 ...
 |
This is the name of the folder where the shaders and the configuration files (cfg) for the engine are.
...
**
 ...
 |
This is the name of the localization folder.
...
**
 ...
 |
This is the name of the root folder where the previous directories are.
...
**
 ...
 |
This is the name of the folder where the compiled assets are temporary outputted. the ${p} symbols is the platform name passed as a parameter to the rc.exe
...
**
 ...
 |
This is the name of the folder where the RC will output the pak files.
...
These are describing, for example, which file types are or are not xml files. Note that you can set new properties or override the default properties from the command line with the
**
myVarName=myValue
**
 syntax.
...
1. The conversion job, defined in-between the tag
**
<ConvertJob> [...] </ConvertJob>
**

 2. The job to perform some extra copy operations defined in-between the tag
**
<CopyJob> [...] </CopyJob>
**

 3. The packing job defined in-between the tag
**
<PakJob> [...] </PakJob>
**

 4. The cleaning job that cleans the target folder defined in-between the tag
**
<CleanJob> [...] </CleanJob>
**
...
Inside each job (ConvertJob, CopyJob, ...) you need to describe which operations needs to be executed. To obtain this goal you can use the tag
**
<Job>
**
. This tag has different parameters that gives RC the exact information about the task he needs to perform. You can read some examples in the following paragraphs but a full list of possible operations is contained in the RCJob attached to this page.
...
Platform
 ...
 |
Example of the command needed to be executed to run an rc job
...
**
 ...
 |
Tools\rc\rc.exe /job=Tools\rc\RcJob_Build_SDK.xml /p=pc /threads=8 > pclog.log
...
**
 ...
 |
Tools\rc\rc.exe /job=Tools\rc\RcJob_Build_SDK.xml /p=xb1 /threads=8 > xb1log.log
...
**
 ...
 |
Tools\rc\rc.exe /job=Tools\rc\RcJob_Build_SDK.xml /p=ps4 /threads=8 > ps4log.log
