...
Most objects should be grey, but as the geometry
gets more complex it's shaded red.
...
You can also modify the distance at which it draws the wireframe overlay on the proxies. Default = 40m from the camera.
If you would like to extend this, you can modify the default value with the CVar...
...
s - show static entities (brushes, vegetation, non-physicalized objects).

r - show sleeping rigid bodies

R - show active rigid bodies

l - show living entities (Players, AI)

i - show independent entities (ropes, boids, cloth)

g - show triggers

a - show areas (gravity volumes, water volumes & wind areas)

y - show RayWorldIntersection and PrimitiveWorldIntersection requests
...
p_draw_helpers y (y -
**
show RayWorldIntersection and PrimitiveWorldIntersection requests)

This view displays all the ray- and primitive-casts that are currently happening.
When investigating RWI/PWI calls you can control how the info is displayed with these CVars.
...
This example show you the physics system related to areas, with bounding boxes + geometry) 1
**
Type
**
 with multiple
...
This example is in the other direction, multiple
**
Types
**
 with 1
...
You can combine these 2 CVars together to which will further break down the entities into their groups. To see what you are spending in ms of each class of entity.
With the top 16 being listed in the top left of the screen, you can teleport to each one with the additional cvar...

**
p_jump_to_profile_ent N
**
(N being 1 - 16)

Additionally, the top 5 entities in the list are bound to the shortcut keys
**
alt+1 -> alt +5
**
.
...
To display joints you have to activate
**
p_draw_helpers 1
**
...
To enable this feature, make sure that you create dumps of the physical world. A CPhysicalWorld function allows you to do that, and also you can use a cvar
*
**
"p_do_step 2"
**
...
worldgeoms.txt
*
 files in the root folder of the build. This allows you to load the dumps into PhysDebugger.exe either as startup params or through the main menu. Additionally, the cvar
*
p_do_step 3
*
 allows you to create a binary dump (world.phump). You can load this binary dump instead of a world
...
**
 |
Default backdoor option to make world dumps.
 |
