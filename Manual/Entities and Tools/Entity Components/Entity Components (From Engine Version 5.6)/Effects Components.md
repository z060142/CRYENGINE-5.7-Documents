...
**
 |
Albedo color of the fog volume, as revealed by external light sources. (with no external light, this will be black)
 |
...
**
 |
Specifies how much luminance (kcd/m2) the fog emits. Only in effect if the Emission color has a luminance greater than 0.
 |
...
**
 |
Whether or not to use the fog volume fog uses the color set globally in the environment editor's Variables → Volumetric Fog → Color (Entities).
 |
...
constrains
*
 the fog volume only to a clip volume if placed inside it, or, if placed
*
outside
*
 a clip volume and
...
**
 |
Direction of the falloff. 0 = East; i.e. a positive X value. Determines how strong the fog loses density in the longitude/horizontal direction.
 |
...
**
 |
Direction of the falloff. 90 = up.
 |
...
**
 |
Controls how much to shift the fog density along the falloff direction.
 |
...
**
 |
Scales the density distribution along the falloff direction. Higher values will make the fog fall off more rapidly.
 |
...
**
 |
The distance at which the fog density reaches the value set by the Global Density property, multiplied by Ramp Influence
 |
...
To load a particle effect, click on the
**
Browse
**
 button; to edit the particle effect that you would like to use, click on the
**
Edit
**
 button.
