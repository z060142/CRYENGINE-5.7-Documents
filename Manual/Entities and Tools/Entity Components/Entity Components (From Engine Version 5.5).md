...
**
 |
Determines to which attribute we want to listen. You will only receive signals for those specific attributes.
 |
...
**
 |
Whether the eye sensor is attached to a bone or the pivot.
 |
...
**
 |
If enabled, vision ray-casts cannot pass through colliders that are part of solid objects (sometimes also denoted as 'hard cover').
 |
...
**
 |
If enabled, vision ray-casts cannot pass through colliders marked as 'soft cover'. These are colliders that only block vision but you can still sit 'inside' them (examples are: bushes, tall grass, etc).
 |
...
**
 |
What agent types' NavMeshes should be affected.
 |
...
**
 -
agent doesn't contribute to collision avoidance.
...
**
 -
agent isn't trying to avoid obstacles but other agents consider him as an obstacle.
...
**
 -
agent is actively trying to avoid obstacles.
 |
...
**
 |
Radius of the cover user located in a cover used to determine which cover locations are blocked.
 |
...
How far
 the cover user
 can be from the exact cover location.
 |
...
**
 |
How long
the compromised cover location
will be considered as not valid for further cover queries.
 |
...
**
 |
Name of the behavior tree (its file should be located under
*
...
**
 |
This trigger gets executed when Play is called.
 |
...
**
 |
This trigger gets executed when Stop is called.
 |
...
**
 |
Stops the PlayTrigger if no StopTrigger assigned - otherwise executes the StopTrigger.
 |
...
**
 |
Whether or not to automatically spawn the decal.
 |
...
**
 |
The material we want to use on the decal.
 |
...
**
 |
Whether or not the decal follows the entity.
 |
...
**
 |
The method of projection to be used.
...
**
 |
The depth at which the decal should be projected.
 |
...
**
 |
Sorting priority - used to resolve depth issues with other decals.
 |
...
**
 |
Whether or not the fog volume is currently active.
 |
...
**
 |
The type of shape to use for rendering the fog volume.
 |
...
**
 |
Lights up the fog volume (GI / Injects light into the global volumetric fog system.
...
**
 |
Specifies how much luminance (kcd/m2) the fog emits.
 |
...
**
 |
Whether or not to use the global fog color for this fog volume.
 |
...
**
 |
Whether the fog volume ignores the use of VisAreas in the level.
 |
...
**
 |
Whether the fog volume can only affect the area it is currently in.
 |
...
**
 |
Controls the density of the fog volume.
 |
...
**
 |
Scales the density inside the fog volume.
 |
...
**
Height Falloff Longitude
**
 |
Direction of the fall off (longitude). 0 = East (Positive X). Determines how strong the fog loses density in the longitude/horizontal direction.
 |

**
Height Falloff Latitude
**
 |
Direction of the fall off (latitude). 90 = up.
 |

**
Height Falloff Shift
**
 |
Controls how much to shift the fog density along the fall off direction.
 |
...
**
 |
Scales the density distribution along the fall off direction. Higher values will make the fog fall off more rapidly.
 |
...
**
 |
Defines the distance where the fog starts to get denser
...
**
 |
Lighting multiplier for when LDR is being used to make lighting fit to HDR lighting manually.
 |
...
**
 |
Same as Ramp Start but where the fog stops being dense.
 |
...
**
 |
Scaler how much Ramp Start / End should influence the fog density.
 |
...
**
 |
Scaleron the noise distribution that generates the fog density.
...
**
Density Noise Time Freqency
**
 |
Controls the time frequency of the noise for the density. High frequencies produce fast changing fog.
 |
...
**
 |
Controls the spatial frequency of the noise for the density. High frequencies produce highly detailed fog.
 |
...
**
 |
Whether or not the particle should emit by default.
 |
...
**
 |
Adjustable values which can be defined in the particle effect itself.
 |
...
**
 |
Advances emitter age to its equilibrium state.
 |
...
**
 |
Particle emission speed multiplier.
 |
...
**
 |
Whether this component will ignore vis areas placed in the level.
 |
...
**
 |
Enables rain around the Entity.
 |
...
**
 |
The size of the area around the entity Entity in which the rain will be falling.
 |
...
**
 |
Defines how dark the puddles and rain are.
 |
...
**
 |
Amount of splashes the rain creates on the ground (it just scales the amount of splashes)
 |
...
**
 |
Same as Random Scale, but for the Strength value.
 |
...
**
 |
Randomly spawns ripples at an offset position relative to the entity. These values set the maximum offset on the X and Y axes (local coordinate system).
...
**
 |
Determines the behavior of the static mesh.
 |
...
**
 |
Determines the character to load.
 |
...
**
 |
Minimum graphical setting at which shadows will be cast.
 |
...
**
 |
View distance from 0 to 100 - where 100 = always visible.
 |
...
**
 |
Level of Detail distance from 0 to 100 0 - where 100 = best LOD.
 |
...
Disabled -
**
Object is excluded from voxelization and mostly does not affect the GI. (But it receives all GI effects from the rest of the scene).

-
**
Static Voxelization -
**
 Object is voxelized allowing in-directional occlusion effects, large scale AO and more precise light bounce.

-
**
Analytical Proxy Soft -
**
 An occluder applied during the cone tracing stage and used like a true replacement of voxels allowing also light bouncing from it. Occlusion fades with distance allowing softer shadow effects.

-
**
Analytical Proxy Hard -
**
 An occluder applied during cone tracing stage and used like true replacement of voxels allowing also light bouncing from it.

-
**
Analytical Occluder -
**
 An occluder applied in a deferred way after all cone tracing is performed.
**
...
in
**
Total Illumination v2 settings
**
 (
**
...
**
 Integrates this mesh into the terrain mesh. This allows support for all terrain material features like 3-planar texturing.
**
Integrate Objects
**
 must be
...
**
 (
**
Tools -> Level Editor -> Level Settings -> Terrain
**
).
...
**
 |
Path to the Mannequin .adb file.
 |
...
**
 |
Path to the Mannequin controller definition file.
 |

**
Default Scope Context Name
**
 |
Name of the default Mannequin scope context to use.
 |
...
**
 |
Name of the default Mannequin fragment to play.
 |
...
**
 |
Whether or not to use root motion in the animations for physical movement.
 |

**
Use Ground Alignment
**
 |
...
Note:
**
 Mass cannot be set at the same time as density.
 |
...
Note:
**
 Density cannot be set at the same time as mass.
 |
...
**
 |
The geom cache file (.abc or .cbc) to load.
 |
...
**
 |
Determines the behavior of the mesh.
 |
...
**
 |
Determines the animated mesh to load.
 |
...
**
 |
Level of Detail distance from 0 to 100 - where 100 = best LOD.
 |
...
**
 |
Specifies the animation to play by default.
 |
...
**
 |
Whether or not to loop the default animation.
 |
...
**
 |
Controls the speed of the animation. Higher value means play the animation faster, smaller value means play the animation slower.
 |
...
**
 |
Whether this component will ignore vis areas.
 |
...
**
 |
View distance from 0 to 100, 100 being always visible.
 |
...
**
 |
Level of Detail distance from 0 to 100, 100 being always best LOD.
 |
...
**
 |
The shape of the physics area. This can be either a box, sphere, cylinder or capsule.
 |
...
**
 |
Flow speed of the medium (air, water) inside the area.
 |
...
**
 |
Whether to use a capsule for the default geometry, otherwise a cylinder wil be used.
 |
...
**
 |
Air resistance for the character (dampens velocity when airborne).
 |
...
**
 |
Maximum ground slope angle the character can climb.
 |
...
**
 |
Maximum ground slope angle on which the character can still jump.
 |
...
**
 |
Minimum ground slope angle before the character starts sliding.
 |
...
**
 |
If the ground slope angle exeeds this value, the character will be forced to lose ground contact and start falling.
 |
...
**
 |
Maximum velocity of the surface the character is on before they are considered airborne and slide off.
 |
...
**
 |
Whether the physics of the entity will be enabled by default.
 |
...
**
 |
Whether or not this component should listen for collisions and report them to Schematyc.
 |
...
**
 |
Engine's RPM at reaching which the engine torque decreases to 0.
 |
...
**
 |
The clutch is disengaged if the engine's RPM falls below this limit.
 |
...
**
 |
Alternative way of setting mass.
 |
...
**
 |
Used as a hint to group wheels (to align wheels on opposite sides of the same axle if they differ only very slightly). Axle < 0 makes the wheel purely cosmetic, i.e. it will be aligned with the ground, but won't affect the vehicle movement.
...
**
 |
Whether the wheel uses raycast instead of full geometry cast (which is simpler and faster, but less realistic).
 |
...
**
 |
Force limit for compression along the joint's z axis.
 |
...
**
 |
Force limit for pulling apart along the joint's z axis.
 |
...
**
 |
Force limit for shift orthogonal to the joint's z axis.
 |
...
**
 |
Torque limit for bending relative to the joint's z axis.
 |
...
**
 |
Torque limit for twisting around the joint's z axis.
 |
...
**
 |
Accumulate this fraction of 'structural damage' each time the joint is affected (this effectively makes each force/torque limit weaker).
 |
...
**
 |
The joint will only break if either of the 2 parts it connects receives an impulse.
 |
...
**
 |
Size of the sampling geometry used to find the parts the joint connects. Also used to re-connect the joint if the entity undergoes procedural breakability.
 |
...
**
 |
Break limit for the dynamic constraint (if exceeded, the constraint will be removed).
 |
...
**
 |
Whether the entities connected by the constraint will ignore collisions between themselves.
 |
...
**
 |
Whether or not the constraint should be added when the component is created.
 |
...
**
 |
The direction of the line the entity will be constrained to (in the component's frame).
 |
...
**
 |
Minimum position that the entity can be moved to from the constraint pivot. A unit of -1 indicates that the minimum position is one meter behind the entity.
 |
...
**
 |
Maximum position that the entity can be moved to form the constraint pivot. A unit of +1 indicates that the maximum position is one meter ahead of the entity.
 |
...
Constrains entity movement to a plane
, relative to another entity. The other entity is passed to ConstrainToEntity function, or it can be 'the world' if ConstrainToPoint is used.
 Rotation can optionally be limited as well.
...
Creates a point-to-point constraint between 2 entities.
The other entity is passed to ConstrainToEntity function, or it can be 'the world' if ConstrainToPoint is used.
Rotation can be constrained in two dimensions: as rotation around the constraint axis and as bending of that axis on one entity's frame relative to it the other entity's frame.
...
**
 |
Axis that's used to set the rotation limits.
 |
