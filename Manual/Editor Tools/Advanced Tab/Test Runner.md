...
The Menu can be accessed via the  button on the top-right corner of the Test Runner. When clicked, it reveals the following
**
...
**
 |
Lists all default and custom toolbars created for this tool, allowing you to select which toolbar you'd like to hide or display.
 |
...
**
 ...
 |
Displays the panels that have been previously closed or not yet opened in the Test Runner.

The following panels can be displayed with this option:
...
**
 ...
 |
Resets the layout. This option is a quick way to go back to the default Test Runner layout.
...
When a single test or a category is
**
right-clicked
**
...
Run selected tests
**
 option which then can be clicked upon to execute these test(s).
...
**
 |
Executes all the tests that are currently available in the Tests list.
 |
...
**
 |
Searches for a specific test based on its name, displaying only the related tests on the list.
 |
...
**
 |
Displays the tests that can be executed.
 |
...
**
 |
Displays the result of the tests that have been previously executed.
 |
...
The test results will be archived in
*
<Project Folder>\user\testresults
*
, as an
**
.xml
