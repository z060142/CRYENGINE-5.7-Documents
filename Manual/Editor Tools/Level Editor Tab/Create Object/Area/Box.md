...
This entity is used to create a box to which the triggers that should be enabled when the player enters or leaves the box and other entities can be linked.
...
**Fade In Zone** | Specifies in meters how big the zone around the box is that is used to fade in the effect attached to the box. Only when the player is inside the box the effect is rendered at 100%, at the beginning of the Fade In Zone its rendered at 0%.
...
**Display Sound Info** | Enable to expand Sound Obstruction options.
