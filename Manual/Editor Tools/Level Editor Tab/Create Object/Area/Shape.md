...
**Height** | Specifies how high the shape area should be, 0 being the infinite height.
...
**Area Id** | Sets up the ID of the area, so areas with another ID can overlap.
...
**Display Sound Info** | Enable to expand [Sound Obstruction](../../../../Audio/Audio%20Overview/Audio%20%26%20Occlusion.md) options.
**Agent Height** | When Render Voxel Grid is enabled this determines the height, along the Y axis, of the grid cells rendered.
**Agent Width** | When Render Voxel Grid is enabled this determines the height, along the X axis, of the grid cells rendered.
**Render Voxel Grid** | If true, Voxel Grid will be rendered when helpers are enabled.
**Voxel Offset X** | Offset Voxel Grid on the X axis.
**Voxel Offset Y** | Offset Voxel Grid on the Y axis.
