...
First the system figures out which scopes are assigned to the requested fragmentID. In other words, it looks up the scopemask of this fragmentID. Typically the fragmentID determines the scopemask by itself, but it is possible to specify 'overrides' and select different scopemasks based on the
...
The order in which the fragments are ranked is displayed in the
[fragment browser](../../Mannequin%20Editor.md#MannequinEditor-Fragments)
 if you deselect the 'folders' button. The best fragments are shown first. The following is an example in which the tag
*
burst
*
