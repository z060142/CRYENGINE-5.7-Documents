...
- Source: https://www.cryengine.com/docs/static/engines/cryengine-5/categories/23756816/pages/29450474
- Page ID: 29450474
- Breadcrumb: Editor Tools > Universal Query System (UQS)
- Parent: Editor Tools
...
-
**
Support for more than just 3D points
**
, for example:
...
-
**
Support for a list of N best resulting candidates
**
, for example:
...
-
**
Support for feeding in parameter values into queries at run-time
**
, rather than being restricted to hard-coded values at design-time only.

-
**
Allows for custom names for query parameters
**
 so that you can better describe the context that the query is intend to resolve, for example:
...
-
**
Custom 'post-validation' for when a query has been completed
**
, for example:
...
Items:
**
Are used to represent the reasoning space that a query is working on. Simply speaking, a query will evaluate a set of items to work out the fitness of each item. Also, they are used as input parameters and return values in functions.

**
Functions:
**
When called, act as input parameters for generators, evaluators and functions themselves. Also, functions can be used to convert between item types or to mix in some custom logic before propagating their return value to the caller.

**
Generators:
**
 Are responsible for generating items. Their purpose is to create the reasoning space via a set of items that will be evaluated by a query. For example, a generator could create a set of points in a grid-like layout.
...
DeferredEvaluators
**
. Both kinds are responsible to "evaluate" an item they have been passed. More precisely, their purpose is to either give the item a score between 0 and 1 (to express the fitness of that item) or to simply discard the item in case it doesn't meet a specific condition.
...
InstantEvaluators
**
 must return their evaluation outcome immediately. They lend themselves well for undertaking quick calculations. A typical use case would be to qualify the distance between 2 points given a reference distance.
...
-
Added helper classes that makes running a query easier from within the code:
`
...
-
Added a helper class for easier setup of the UQS when using the XML data source:
`
...
Dir3
`
. Functions, generators and evaluators using the old data type have been adjusted accordingly.
