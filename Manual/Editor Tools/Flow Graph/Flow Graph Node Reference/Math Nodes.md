...
Float
 |
Inverse sine (Arcsinus) of the input
 |
...
Float
 |
Inverse tangent (ArcTangens) of the input
 |
...
Math:Auto3DNoise
**
 defines where exactly (on X, Y and Z axes) the floating points should be generated and how high/low the curve values should be. These values are based on the
**
Frequency
**
 and
...
Boolean
 ...
 |
If true, the node will output values constantly at specific intervals.
...
Vec3
 ...
 |
A Vec3 seed value from which randomization will occur, allowing each axis to be set independently.
...
Float
 ...
 |
Specifies the time interval at which noise must be sampled. This is also the initial sample time if the Active port is FALSE.
...
Vec3
 ...
 |
Scale factor for input value; defines how often the randomized floating point values will be generated. A lower Frequency would narrow down the range of the value from which the next randomized value can be generated, while a higher Frequency would expand that range.
...
Vec3
 ...
 |
Scale factor for noise values; defines how high/low the randomized floating point values will be. A lower Amplitude would narrow down the range of the value from which the next randomized value is generated, while a higher Amplitude would expand that range.
...
Float
 ...
 |
How often the value outputs if active (in seconds).

 |

**
Rest Internal Time On Deactivate
**
...
Boolean
 ...
 |
If the node is deactivated (by the Active port), internal time resets to 0.0f.
...
Generates
continuous
noise as floating point values, smoothly
interpolating between a series of randomized
...
Boolean
 ...
 |
If true, the node will output values constantly in a certain interval.
...
Int
 ...
 |
An integer seed value from which randomization will occur, allowing each axis to be set independently.
...
Float
 ...
 |
Time at which to sample noise. This is also the initial sample time if the Active port is FALSE.
...
Boolean
 ...
 |
If the node is deactivated (with the active port), internal time resets to 0.0f.
...
Float
 ...
 |
Randomized floating point value.
...
*
 ...
 |
The input signal to be converted
...
Boolean
 |
True if the input is within the range
 |
...
Any
 |
Triggered if the input is within the range
 |
...
Any
 |
Triggered if the input is outside of the range
 |
...
Float
 |
Scalar Input value to sample noise at
 |
...
Float
 |
Multiplication of X by Frequency and Amplitude values
 |
...
Vec3
 |
Vector input value to sample noise at
 |
...
Float
 |
Multiplication of V by Frequency and Amplitude values
 |
...
Any
 |
Resets PortCount and TotalCount
 |
...
Integer
 |
Number of ports that have been set
 |
...
Integer
 |
Sum of all times any of the input ports have been set
 |
...
Boolean
 |
Triggered when PortCount reaches PortThreshold
 |
...
Boolean
 |
Triggered when TotalCount reaches TotalThreshold
 |
...
*
 ...
 |
The second number to subtract from the first
