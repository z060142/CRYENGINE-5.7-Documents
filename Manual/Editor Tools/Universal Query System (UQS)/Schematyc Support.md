...
`
 |
Name of the query blueprint that shall be started.
 |
...
`
 |
For debugging purposes only: name of the querier to identify more easily in the query history.
 |
...
`
 |
Name of the query's runtime parameter.
 |
...
`
 |
The actual data of the runtime parameter.
 |

##
UQS::StartQuery

Actually starts the query that has been set up so far via
**
UQS::BeginQuery
**
 and the various
...
`
 |
A unique ID to identify the freshly started query. This can be used later for fetching the resulting items once the query finishes.
 |
...
`
 |
The ID of the query that UQS::StartQuery returned.
 |
...
`
 |
Index of the item that you want to fetch. This can be between 0 and the ResultCount of UQS::QueryFinished.
 |
...
`
 |
First QueryID to use by the comparison.
 |
...
`
 |
Second QueryID to use by the comparison.
 |
...
`
 |
True/false depending on whether the two QueryIDs are equal or not.
 |
...
`
 |
ID of the query that has just finished (it's an ID that a previous call to UQS::StartQuery returned).
 |
...
`
 |
Number of resulting items (this may even be 0 if no item was found by the query).
 |
...
`
 |
Some text explaining what went wrong.
 |
