...
**
 |
The whole query bails out with an exception.
 |
...
**
 |
Only the current item will be discarded (marked as having encountered an exception).
 |
...
**
 |
The function will stop calling further child functions and propagate the exception up to the caller (i. e. Generator or Evaluator) from where it gets handled.
 |
