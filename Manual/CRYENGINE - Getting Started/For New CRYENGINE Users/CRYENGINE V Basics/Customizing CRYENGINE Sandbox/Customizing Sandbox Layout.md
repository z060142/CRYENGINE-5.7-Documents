...
**
 Flow Graph
**
 window is used, but of course this applies to any window.
...
If the Sandbox layout needs to be reset to the default settings,  choose
**
 Layout -> Reset Layout
**
. This will undo all the changes made to the layout and show only the toolbars that are shown by default.
...
Do this by copying its *.json file from your
*
local
*
 project folder (by default
`
C:\Users\<User_Name>\AppData\Roaming\Crytek\CRYENGINE\Layouts
`
) to the
`
Engine\Layouts
`
folder in your game folder. The latter folder may need to be created as it doesn't exist by default.
