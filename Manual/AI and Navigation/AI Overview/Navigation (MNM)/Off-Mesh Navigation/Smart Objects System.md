...
**
 |
The settings in this category define the user of a rule.
 |
...
State Pattern
 |
State pattern to match on the user class. Only users whose state matches the state pattern will be considered.
 |
...
**
 |
Contains all settings that define the object of a SmartObject rule.
 |
...
State Pattern
 |
State pattern to match on the object class. Only objects whose state matches the pattern will be considered.
 |
...
Randomness
 |
This value sets how much randomness will be added between the minimum and maximum delay.
 |
...
**
 |
The additional settings in this category are used to further define the rule and emphasize certain aspects.
 |

Proximity
 |
This value sets the importance of the proximity of the object to the user. This value can be used for the most far away objects being the first to be used by the user.
 |
...
Visibility
 |
This value determines how important the visibility is between the user and object.
 |
...
**
 |
Not only the action itself can be selected in this category, but also the pre and post action states. The pre action states are set before the action is started and prevents it being started multiple times.
...
Approach Helper
 |
Object's helper point to approach before the animation plays. If empty, the animation's helper is used.
 |

Start Radius
 |
Distance from the Smart Object start point required for starting the animation.
 |

Direction Tolerance
 |
Maximum difference between the AI and smart object start point orientation - if the AI is not oriented like that, it's rotated before starting.
 |
...
Edit
**
 in the Smart Objects Editor window. A dialog window now appears. If no SO class is selected it means that the entity is not yet registered as a smart object and if so, start AI/Physics and exit it to refresh the system. Deselect and select the entity once again and repeat this step.
...
OK
**
. The helper will be created and (if the entity is still selected) appears in the editor's main view as a small yellow wire box and by default, is already selected. When the mouse is pointed over the box, its name will be shown next to it.
...
ActionEnd
**
. These nodes are used to start and end the AIAction.
...
ActionStart
**
 node outputs the ID's of the user and the object on the according ports. This pulse can be used to trigger all necessary nodes, just like in a normal graph.

##
User and Object IDs

There are two ways of selecting the target entity for a node in an action graph: Connecting the
**
ActionStart
**
 output ports to the according ID input port, or selecting the target entity from the context menu of the entity.
