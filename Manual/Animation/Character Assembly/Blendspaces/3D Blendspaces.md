...
**
. Just compare the first ten entries in the <ExampleList> and the first eight entries in <ExamplePseudo>. The only difference is that we also added 6 slope assets. To annotate a 3D blend-space we use volumes like tetrahedrons, pyramids and prisms.
