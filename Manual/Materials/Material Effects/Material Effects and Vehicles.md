...
To get started lets copy and modify the existing vehicles.Common.Ground.Light.Mud PFX and save it under vehicles.Common.Ground.New.NewMud. It now looks something like this:

![Image](https://www.cryengine.com/docs/static/attachments/35402303) *It's a good idea to create your PFX against an easy-to-distinguish background, like a Designer object.*
...
Using Excel, open the file located in: `<GameFolder>\Libs\MaterialEffects\MaterialEffects.xml`

If you scroll down a bit you will find the following block of information:

![Image](https://www.cryengine.com/docs/static/attachments/35402304) *Disregard the inclusion of the "SCAR" row for this tutorial.*
...
Shortly we'll explain how exactly these setups all communicate, but for now, create a new row underneath (doesn't technically matter where) the vfx_HMMWV and call it "vfx_CarNew".

Also add a new column called mat_mud (typically you would arrange these better, in groups).
...
As mentioned just above, we've created a new *reference* to a new surface type, but that surface type does not yet exist, you cannot assign it to any materials yet.

To create a new surface type simply open the file: `<GameFolder>\Libs\MaterialEffects\SurfaceTypes.xml`
...
Segue
At this point, this may seem a little overkill just to add some new mud effects, but by the end of this you'll hopefully understand and appreciate the immense amount of flexibility this system offers. Don't forget that you can of course always copy/paste existing content to give you a head-start and existing setups can always be expanded on, rather than needing to create brand new setups!
...
Open the following file: `<GameFolder>\Libs\MaterialEffects\FXLibs\vehicles.xml`
...
Open up the HMMWV vehicle script here: `<GameFolder>\Scripts\Entities\Vehicles\Implementations\Xml\HMMWV.xml`
...
Now lets modify the HMMWV script to see if our setup works. Change the mfxRow parameter to use our new "vfx_CarNew" row.
...
![Image](https://www.cryengine.com/docs/static/attachments/35402309) | ![Image](https://www.cryengine.com/docs/static/attachments/35402310)
--- | ---
New Mud Effect | No Grass Setup for the vfx_NewCar row!
![Image](https://www.cryengine.com/docs/static/attachments/35402311) | ![Image](https://www.cryengine.com/docs/static/attachments/35402312)
Testing New Surface Types | Particle System only bound by your creativity!
