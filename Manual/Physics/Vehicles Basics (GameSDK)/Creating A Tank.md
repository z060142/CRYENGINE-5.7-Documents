...
Tanks need special treatment because they are a combination of.cga and.chr files. For general information about vehicle setup, please refer to [Vehicle System](/docs/static/engines/cryengine-3/categories/1638401/pages/1605652).
...
- The vehicle needs to *look* down the positive y-axis (-Z for Maya).

- The tank treads need to be skinned characters and exported as a chr *separately* from the rest of the tank because the tank itself will be exported as a cga file.
...
- Create a helper in the middle of the tank tread. Give it the same name as the tank tread, plus the suffix (example: *tread_left_root*).
- Create a 2-link bone chain for each tank tread wheel and position them like in the picture below. The first bone of the wheel needs to be named like the wheel plus the suffix (example: *wheel01_bone*). The child (terminator bone) needs to be named like the wheel, plus the suffix (example: * wheel01_term*).
...
For more information please see the [Vehicle Editor](../../Editor%20Tools/Deprecated%20Tab/Vehicle%20Editor.md) article.
