...
Code
**
: Surface defined in the SurfaceTypes.xml (this points to the particular boolean shape to be used for subtractions).

-
**
Code
**
: Surface effects defined in MaterialEffects.xml.
...
Code
**
: Shape object registered in physics.lua.
...
`
. It is not necessary for example, to build splinters, but these could be used later to create the particle effects around the breaking part.
...
breakable_id
**
 number is whatever number you defined in the breakability index in your Physics.lua file.
...
Note
The surface type includes only an explosion shape id. The actual mesh is registered in the
`
Game\Scripts\physics.lua
`
 file.
