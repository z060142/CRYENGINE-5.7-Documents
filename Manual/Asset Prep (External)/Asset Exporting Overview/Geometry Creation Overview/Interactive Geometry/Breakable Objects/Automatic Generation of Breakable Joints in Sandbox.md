...
**
 |
Allows to specify the alignment of the break template relative to the source object's node. For random alignment, AlignmentSeed can be used to make random numbers reproducible (if several JointGens affect the source object, either only one of them should have a seed, or all seeds must be the same)
 |
...
**
 |
Equivalent to adding 'entity' keyword to each tpl piece's UDP. Use it if you have problems with particles for one reason or another
 |
...
**
 |
Enables hierarchical breaking for cut parts (generated pieces will remain hidden until any of them break, which is good for performance).
 |
...
**
 |
An impulse of this strength, applied at the jointgen's location directed towards the source's surface, will cause breaking
 |
...
**
 |
Radius of the sphere used to check collisions with the source object
 |
...
**
 |
By default, the template's longest and shortest dimensions are aligned with that of the source piece and scaled to be a little bit larger. This scale can apply additional scaling if necessary
 |
...
**
 |
Optional template offset relative to the aligned position (can be used if the default position produces unwanted results)
 |
...
**
 |
Pieces can be made to disappear after this timeout (0 disables)
 |
...
**
 |
Similar to Timeout, but only ticks when a piece is invisible
 |
...
**
 |
Disables Timeout on pieces with volume larger than this
 |
...
Once the joints are generated, they can immediately be tested in game in the original entity/brush. It's useful to have the corresponding physics helpers enabled during testing, and
**
p_debug_joints
**
, which will show pieces' masses and joints' hit-points. Also, generation outputs the summary report in the log.
