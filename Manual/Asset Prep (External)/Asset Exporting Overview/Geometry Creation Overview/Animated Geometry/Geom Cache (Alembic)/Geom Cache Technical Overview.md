...
Link to asset creation documentation for GeomCaches can be found [here...](../Geom%20Cache%20(Alembic).md)
...
- Gets an update each frame from the engine (CGeomCacheManager::StreamingUpdate)
...
**e_GeomCacheBufferSize** | Sets the size in MB for the streaming buffer heap. Default is 128MB, which is enough ~10MB/s. Other values are conservative in Ryse, could potentially be enough for more.
...
**e_GeomCacheMinBufferAheadTime** | The minimum amount of time the manager will always try to keep in memory for each stream. Lower values will result in a higher likelihood of stream aborts. If this is already reached there will be no more disk requests issued for the frame.
...
**e_GeomCacheDebugDrawMode** | 1: Only animated meshes 2: Only static meshes 3: Displays the same render meshes with the same color. Useful for debugging instancing. Note that 1 and 2 will only have effect when the cache is playing at the moment.
