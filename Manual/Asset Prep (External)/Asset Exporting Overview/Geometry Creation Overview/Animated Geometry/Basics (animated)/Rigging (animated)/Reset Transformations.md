...
`
10 units, and object B is at X
`
 15 units. If neither object has a parent, then the world is the parent and the X value shown for A is 10, and the X value shown for B is 15. These values are relative to the world origin (0,0,0).
...
4. Link
controllerZeroHelp
 to the parent of the controller you want to zero out.

5. Link that controller to
controllerZeroHelp
.

6. To make it easier to deal with, turn off the Axis Tripod option for the helper, leaving on the Center Marker only.

7. Now select the controller you want to zero out. On the Main toolbar, choose
Parent
 as the reference coordinate system. The values are all zero. The same values will be shown in the Motion panel and Track View.
