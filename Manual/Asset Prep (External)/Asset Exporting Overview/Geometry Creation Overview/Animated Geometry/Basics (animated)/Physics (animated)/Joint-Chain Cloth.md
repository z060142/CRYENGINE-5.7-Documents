...
p_max_bone_vel
**
 cvar to limit the velocities the physics solver thinks the bones move with (and which it thus tries to match). if you are experiencing rough behavior on fast animations with the default value, try reducing it to 3 or 4.
