...
- Source: https://www.cryengine.com/docs/static/engines/cryengine-5/categories/23756816/pages/29450849
- Page ID: 29450849
- Breadcrumb: Asset Prep (External) > Asset Exporting Overview > Geometry Creation Overview > Animated Geometry > Additive Animation
...
**
 and load the bip file
**
relaxed_run_nw_forward_fast_01
**
 in
**
Mixer.
...
-
The arm animation has an upper-body animation that replaces the existing left arm animation. Because it is additive, it cannot terminate the existing animation and you only need the left arm to stay in place. By doing so, you also see in this tutorial how you can load multiple animations on different layers in CRYENGINE 3 and beyond.
...
Bone Export
**
 and export the entire arm animation (the name
**
arms_coverFire_01.caf
**
 is used here).
...
**
 Bone Export
**
 panel and export the animation to the additive folder (the name
**
additive_walk_forward_fast_CF_01.caf
**
 is used here).
...
**
 and load the
**
sdk_character_male.cdf
**
 (if it does not load on default).

Check that your new animations show up on the left side of the Character Editor. If you test the additive animation, don't be scared about any deforming that you see. As it is an additive animation, you are not supposed to play it on the Primary layer.
...
You can actually adjust the weights of the Additive Animation. If you go back to Layer Secondary .1, you can set the
**
