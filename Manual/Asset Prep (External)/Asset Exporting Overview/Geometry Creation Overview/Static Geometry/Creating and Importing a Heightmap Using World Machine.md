...
![Image](https://www.cryengine.com/docs/static/attachments/29934046)

## Overview

## Sections

This tutorial will show you how to create a terrain heightmap and colormap with World Machine and and use it in CRYENGINE.

We will use the Free version of [World Machine](http://www.world-machine.com/) (WM) to create the content so everyone can see the process in action. The free version is limited to export out a max 512x512 image, so we will stick with these dimensions.
...
The CRYENGINE part of this tutorial relies on textures used in the GameSDK Sample Project. We recommend that you download this from the [Asset Database](https://www.cryengine.com/marketplace/product/crytek/cryengine-gamesdk-sample-project), import it into your Launcher, start it from there and then create a new level.
...
[Sections](#sections)[Tutorial Files](#tutorial-files)[Setting Up World Machine](#setting-up-world-machine)[Creating the Heightmap](#creating-the-heightmap)[Creating the Colormap](#creating-the-colormap)[Setting Up the Level in CRYENGINE](#setting-up-the-level-in-cryengine)[Importing the Heightmap](#importing-the-heightmap)[Importing the Colormap](#importing-the-colormap)[Finishing Off](#finishing-off)
...
First off, let's set the scale in World Machine under World Commands -> Project World Parameters.
...
Always use 16-bit variations, as they give much better terrain quality than 8-bit BMP format, which suffers from a lot from "[terracing](http://en.wikipedia.org/wiki/Terrace_%28agriculture%29)".
*Pic2: Supported file types*![Image](https://www.cryengine.com/docs/static/attachments/25523524)
...
- Finally we have the Overlay View node. While it's not required, it helps you visualize the height and color maps together inside WM. This is also found in Devices -> Output -> Overlay View.

Connect them up as shown below, with the logic coming out of the back of the Terrace node.

*Pic3: Node connections*![Image](https://www.cryengine.com/docs/static/attachments/25523525)
...
*Pic4: Picking preset in colorizer node*![Image](https://www.cryengine.com/docs/static/attachments/25523526)

#### Bitmap Output Node

Double-click the Bitmap Output node to open its properties. The file type that CRYENGINE accepts for the terrain colormap is BMP.

Set the File Format to BMP. Click the **W****rite output to disk!** button to create the file then click OK.

*Pic5: Bitmap Output properties*![Image](https://www.cryengine.com/docs/static/attachments/25523529)
...
*Pic6: Creating a new level*![Image](https://www.cryengine.com/docs/static/attachments/26512231)
...
Pick the heightmap (hm.r16) file we just created from WM (by default it should be in:`Documents\World Machine Documents`) and click OK.
...
*Pic7: Imported heightmap*![Image](https://www.cryengine.com/docs/static/attachments/25523531)
...
*Pic8: Re-imported heightmap*![Image](https://www.cryengine.com/docs/static/attachments/25523533)
...
Currently we are using the default terrain material (black and white checkbox). To change the terrain texture, we need to change two things: the low detail and the high detail texture and materials. See [Terrain Editor](../../../../Editor%20Tools/Terrain%20Editor.md) for more information.
...
*Pic9: Importing Terrain Texture*![Image](https://www.cryengine.com/docs/static/attachments/26512232)

- Click on the image (with 256 in the middle). This is one terrain texture tile, currently set to 256 which is ½ of our terrain texture resolution. Click the Change Tile Resolution button and pick 512x512 from the drop-down list. If nothing happens, make sure you have highlighted the tile by clicking on it (it will show as grey when selected).
...
- With the tile still selected, click Import and select your terrain texture created from WM. Once selected, click Close on the Import tool and the terrain color will update. Fly up high and check the alignment of the colormap on the terrain. Is it rotated? Does the snowy part follow the highest ridge in the map? If not rotate the image 90 degrees clockwise in Photoshop.

Now your terrain should look something like this:

*Pic10: Colormap imported*![Image](https://www.cryengine.com/docs/static/attachments/25523535)
...
*Pic11: * Paint button**![Image](https://www.cryengine.com/docs/static/attachments/25497055)
...
If you create your own terrain textures/materials, make sure you set the shader type to Terrain.layer from the default Illum Shader.
...
- Select and highlight the first layer in the list (default) and click the Folder button next to Material at the bottom:

*Pic12: Material folder button*![Image](https://www.cryengine.com/docs/static/attachments/25497057)

- This will let you select a material for your layer. Browse under the `Materials\terrain` folder and you can see the available terrain materials.
- Select or highlight in the material editor grass_1 and click Open. This will apply that material to the terrain layer.
...
- Now we will define some rules to this terrain layer. We will only change the Max Angle here to 30 degrees, so when we paint with this layer later, it will not try to paint on any surface slope that is greater than 30 degrees.
...
- Next we will add another Terrain Layer. Right-click in the section of the Terrain Editor where your "Grass" layer is and choose Create Layer. Give it the name "Rock".
- Click once again on the Folder button and pick a rock material like cliff_island_3d.
- Define the rules once again, but this time set the Min Angle to 30 degrees.
- Right click on the name of the layer at the top of the Terrain Editor and select Flood Layer.
...
*Pic13: Terrain Paint button*![Image](https://www.cryengine.com/docs/static/attachments/25497058)
...
*Pic14: Layer painter*![Image](https://www.cryengine.com/docs/static/attachments/25523536)
...
*Pic15: Generating the terrain texture*![Image](https://www.cryengine.com/docs/static/attachments/25523538)![Image](https://www.cryengine.com/docs/static/attachments/25523539)
...
*Pic17: End result*![Image](https://www.cryengine.com/docs/static/attachments/25523543)
