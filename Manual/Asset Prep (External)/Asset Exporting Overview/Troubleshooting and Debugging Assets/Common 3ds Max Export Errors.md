...
**
 When
some faces or objects in your scene have no Crytek shader assigned to it,
you will get the following error:
*
The following material is not set to the Crytek Shader: Face
*
.
...
Resolution
**
: To fix this error, select the object or faces and in the Material Editor and change the Shader Type to
**
Crytek Shader
**
.
...
Issue:
**
 When you export multiple assets in your scene from 3DS Max, and import it into CRYENGINE, you can see all the assets are with one node and only as one file.

**
Resolution:
**
 You need to make sure that the checkbox
**
Export file per node
**
 is checked to avoid exporting multiple dummy nodes as as single file. For example, if you have different pieces of a building that you need to export into the CRYENGINE.
...
Issue:
**
 3DS Max crashes instantly, while exporting your assets. One of the main reasons could be that the LODs have been linked under the dummy node.
...
Resolution
**
: You need to link the LODs under the main mesh node.
...
Issue:
**
 The following error is displayed when you export assets in the CRYENGINE Exporter window, "
*
Failed to compile geometry in node......- More than two triangles share an edge
*
". It means that you might have double faces attached to each other or two faces which are attached sharing the same edge but the vertices are not welded.
...
Modify
**
 tab, and then under Modifier List drop down menu, select
**
STL Check
**
.
...
When you export your asset out of 3ds Max and the
exported *.cgf file size
 is very high
.
...
Resolution:
**
You should check if the vertex animation is turned on for
the scene.
 If it is turned on, make sure to uncheck the box and export the asset again.
...
degenerate faces
*
. Degenerated faces are triangles where all edges lie over each other, which looks like the triangle has no face area.
...
-
Select the meshes that you would like to check, and then under the CryMaxTools menu, select
**
ToolBox.
**
This opens a new window with the meshes selected,
*
...
**
 select
**
 Mark Degenerated Triangles
**
. The tool will go through all selected objects to find degenerate triangles, and place a point helper on them.
