...
The method of linking the camera to the character works fine for **rough blocking** where the actor has no animation applied in the Track View. By attaching the camera to the character and positioning it close to the camera bone (eye level), we can do rough blocking or experiment on the character's point of view and the pace of the scene.

Caution
This method is only good for doing rough blocking. However, it is **NOT** the best way to carry this method until the final process unless the character is being animated in trackview only (f.ex. vehicle moving, and the camera is mimicking the front seat viewport).
...
[Link the camera to the character](#link-the-camera-to-the-character)[Link the camera to the character's camera bone](#link-the-camera-to-the-characters-camera-bone)[Link the camera to the tagpoint](#link-the-camera-to-the-tagpoint)[Link the camera to the character](#link-the-camera-to-the-character)[Link the camera to the character's camera bone](#link-the-camera-to-the-characters-camera-bone)[Link the camera to the tagpoint](#link-the-camera-to-the-tagpoint)
