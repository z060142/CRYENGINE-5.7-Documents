...
- Go to **Create Object -> Legacy Entities -> Physics -> RigidBodyEx** and drag it into your scene.
- Select the Spawnpoint entity, go into **Flow Graph**, make sure the * flowQuickStart* graph is open and click **RMB** to ** Add selected entity** for the Spawnpoint to be added with its properties.
- For this part we will be interacting with the points of the rope so that we can move our other point to above the ball.
In order to edit, click**Edit Shape** in the **Properties** panel and use the **Move** tool (**1** Key) to move the point into place as shown below.
- Grab a primitive box from **Create Object -> Brush -> Default**. Make sure you DO NOT select the box with no proxy. Placing this above the rope top point should show a green box when the physics is active.
- Finally, you can jump into your game (**Ctrl+G**) and shoot the sphere to see that the rope will hold and fling around in the air until it comes to a resting halt naturally.
