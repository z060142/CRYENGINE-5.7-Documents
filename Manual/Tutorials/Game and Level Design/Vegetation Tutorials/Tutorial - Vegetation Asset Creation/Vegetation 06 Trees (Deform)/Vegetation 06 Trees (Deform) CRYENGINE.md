...
>
`
If the export process went successful for the material and geometry, you should have the material as shown in the below images. Note the correct folder structure and also the same SubId setup we configured inside 3dsMax. A lot of the fields have been filled out already through the export process (note the
**
Diffuse
**
 and
...
Normalmap
**
 texture paths already filled in). But there is still a few options that we need to set ourselves.
...
Lighting Settings,
**
 we can adjust the color parameters. To be correctly in line with the PBS pipeline, we have to:
...
>\Assets\
`
textures\detail_bumpmaps\wood\wood_bark_detail.tif>
`
...
Detail gloss scale
 |
Intensity of the detail gloss map
 |
...
This tab in the Material Editor will show you different options depending on the shader type you assign to this material SubId. Since we have selected the vegetation shader, it has exposed the options that apply to the vegetation. To continue, check the box for the
**
Leaves
**
...
**
 |
Uses a cheaper & faster rendering pass on the asset.
 |
...
As you may have noticed, we jumped to this section in favor of the
**
Shader Generation Params
**
. We wanted to enable the
...
If you enable
"UseTerrainColor"
 in the Vegetation tool for the object, this will absorb some of the color information of the underlying terrain where the vegetation object has been placed. This nice feature helps distant vegetation objects blend into the scene at distance
 |
...
>\Assets\
`
objects\tutorial\vegetation\06\tree\tutorial_merged_mesh_deform\tutorial_merged_mesh_deform_leaves_sss.tif>
`
*
...
Nodraw
**
" (because this geometry is not to be shown, it's an invisible trigger).
...
O
`
bjects\tutorial\vegetation\06\tree\merged_mesh_deform\tutorial_merged_mesh_deform>
...
-
To be more precise, don't select the paint objects button (so it's not active) and on the terrain
**
Shift+LMB
**
...
Note
**
: Don't forget to activate the Wind vector and Breeze generation in the Environment bar to get some general movement
*
*
*
*
