...
Initially, we need to prepare the material that will be applied to the object. This enables us to provide a visual reference to the texture that you are using when modeling or shaping the geometry planes for the grass patch. For the individual grass and the grass patches, you only require one material
*
SubID
*
 for the visible portion of the mesh. Also, a physics proxy is not required on the object as the player is supposed to pass straight through the object without hindrance.
...
Pic3
*
) on the above, on the right side, note under the
**
...
-
Since we are not physicalizing the object, the
**
Physicalize
**
 checkbox has
...
`
We are not adding a normalmap (ddna). This is intentional, since we want our grass asset to be as cheap as possible and we achieve this by enabling the
**
grass
**
 flag in the
...
grass
**
 flag runs the shader through a cheaper pipeline and the main benefit of this is that it does not need a normalmap.

Even though you will be able to add one, but it won't be used. Hence, it's a waste if grass is ticked in the
**
Shader Generation
**
 params.
...
Inside 3dsMax, configure the asset to display the vertex colors. Right click on the model and select
**
Object Properties
**
...
-
Select all the bottom vertices in the
**
Viewport
**
.
...
Paint all
**
 button to flood the black color onto the selected bottom vertices.
...
-
Add the root node of the geometry asset to the export list (in this case it is called "
**
tutorial_grass_patch
**
").
