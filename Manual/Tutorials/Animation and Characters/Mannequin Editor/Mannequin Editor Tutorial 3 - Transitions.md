...
*
(also called Transition Browser) from the left side of the CryMannequin editor.
...
any other fragment
*
, you would leave the "To" FragmentID field blank. Similarly, if you keep all tags unchecked it means you don't care about which tags are set.
...
Earliest Start Time
*
. This can be hard to envision, so let's work on an example.
...
Cyclic Locked
*
 flag. From then on the earliest start time is stored in a 'cyclic' way: the time restarts at 0 after each cycle.
...
part
*
 of the action that is "using binoculars" instead of as a part of the next action. If programmers ask for this, you will have to mark these kinds of transitions as "Outro". This won't change the behavior in the editor, but it will change the behavior of game code.
