...
As C++ projects get larger, adding more functions, events and general code, you’ll find that the code can quickly become unorganized. Therefore, it’s common practice to periodically refine your code before adding new functions. Your
*
Player.h
*
 and
...
Player.cpp
*
, while not incredibly long, could use some refinement and renaming in areas to make better sense of what we currently have. This will make it easier to locate bugs and communicate about your code with others.
...
Game.sln
*
, which can be found within the solution folder of your project's main directory, in Visual Studio (or your editor of choice).
...
private
`
 class, add the following values:
...
-
T
his has defined the values for Schematyc, but now you must update the
`
AddMember
`
values to use these constant values. This is done by changing the end value of each
`
AddMember
`
line to the one defined above in the new
`
private
`
...
private
`
 class by adding comments defining them.
...
Since these runtime variables will be used to refine the camera in
*
Player.cpp
*
, you do not need an
...
AddMember
`
line or definitions for these with variables.
...
protected
`
class, add the following functions that will later be used to clarify code function in
*
Player.cpp
*
.
...
With the header cleaned up, you can move on to refining
*
Player.cpp
*
 by clearly defining your logic by name and implementing some of the new variables added to
*
Player.h
*
.
...
namespace
`
, create a definition and name for
`
CPlayerComponent
`
.
...
-
Beneath that, add the variables declared in
*
Player.h
*
, followed by a function body (defined by a pair of curly brackets):
...
Event
`
 (as it currently is) to a Function, you can call it within other section of the code.
...
EEvent::Reset
`
 and paste them into the body of
`
...
Reset
`
by adding a line to call it within
`
CPlayerComponent::Initialize()
`
:
...
EEvent::Update
`
, replace the existing logic with the functions you added to
*
Player.h
*
:
...
`
 as
`
CPlayerComponent::UpdateMovement
`
*
...
UpdateRotation();
`
 by using by adding the following below
`
CPlayerComponent::UpdateMovement:
...
, then select
**
Create Definition of 'UpdateRotation' in Player.cpp
**
.
...
include
`
 that will be used to determine if the character is currently under a physical object while crouching.
...
Player.h
*
 and add two new member variables under the
`
//Runtime Variables
`
 section:
...
AddMember
`
 lines for each of these new values:
...
Player.cpp
*
, add the following variables to the list of
`
CPlayerComponent
`
variables
...
Now you will need to define the main logic involved in having your player crouch, which will be controlled by the
**
Left Ctrl
**
 key. This tutorial creates a crouch controlled by a hold-key, a key you press continually to activate the effect. Therefore, the code must check for both the key press, the key release, and if the character is already crouching.
...
`
 line
 and modify it so that you have a unique
`
eKI
`
 (LCtrl) and name for the crouch action:
...
RegisterAction
`
, start a new line to input your logic.
...
if
`
 statement, add the following else if statement to set the standing function.
...
float frametime
`
 to the brackets of the initial call for
`
...
TryUpdateStance
`
 right before the performance updates to
*
Player.cpp
*
.
...
**
 of "Refining
Player.cpp"
, so that our updates happen only after we have actually pressed the crouch key.
...
TryUpdateStance
`
(still within the curly brackets):
...
CharacterController
`
 which will detect intersections and prevent the player from standing, even if
**
LCtrl
**
has been released.
...
**
 properties,
 where your defaults should now be pre-set.

-
Play around with the values in the
**
CPlayerComponent
**
